...
const bubbles = [
  { text: 'hey! 👋' },
  { text: 'I worked in healthcare, so naturally, ive been drawn towards messy problems and fixing them' },
  { text: 'turns out, healthcare had an even messier one:  technology.  now im here studying HCI' },
...
const cases = [
  {
    ...
    experiences: {
      ...
      glassesResearch: {
        ...
        visual: { src: 'images_fl/research2.png', alt: 'Findings from smart glasses capture-quality testing.', caption: 'Low-res: what the glasses\' camera actually sees' },
      },
      glassesIdeation: {
      ...
      glassesFinal: {
        ...
        imgs: [
          { src: 'images_fl/finalsolution3.gif', alt: 'Phone screen showing the capture button that triggers a photo through the glasses\' camera.' },
          { src: 'images_fl/finalsolution4.gif', alt: 'Result screen showing the identified product and an allergy warning if there\'s a match.' },
        ],
      },
      glassesOutcomes: {
...
function openCase(i, push = true) {
  ...
  flowsEl.querySelectorAll('.cs-sol-fig video').forEach(v => {
    ...
    v.addEventListener('click', () => {
      openLightbox(v.currentSrc || v.src, v.closest('.cs-sol-fig').getAttribute('aria-label'), true);
    });
  });
          ...
          <div class="cs-research-visual">
            <figure class="cs-rfig">
              <button type="button" class="cs-rfig-btn" aria-label="Expand image: ${(grVisual.alt || '').replace(/"/g, '&quot;')}">
                ...
                <span class="cs-rfig-zoom" aria-hidden="true">⤢</span>
                ${grVisual.caption ? `<span class="cs-rfig-pill">${grVisual.caption}</span>` : ''}
              </button>
            </figure>
          </div>` : ''}`;
